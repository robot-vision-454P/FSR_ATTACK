...
def train(epoch):
    ...
    with tqdm(total=(len(trainset) - len(trainset) % args.bs)) as _tqdm:
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            if(is_attack):
                is_attack_target =torch.ones_like(targets)
                net.eval()
                adv_inputs = attack.perturb(inputs, targets, True)
                net.train()
            else:
                is_attack_target = torch.zeros_like(targets)
                adv_inputs = inputs

            adv_outputs, attack_outputs, adv_r_outputs, adv_nr_outputs, adv_rec_outputs = net(adv_inputs, is_attack = is_attack)

            # adv classification loss
            adv_cls_loss = criterion(adv_outputs, targets)
            
            # attack classification loss
            attack_cls_loss = 0
            if (is_attack):
                attack_cls_loss = criterion(attack_outputs, is_attack_target)
            
            # separation & recalibration losss
            ...
            # total loss
            loss = adv_cls_loss + attack_cls_loss + sep_loss + rec_loss 
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            if(is_attack):
                attack_cls_losses += attack_cls_loss.item()
            ...
            _tqdm.set_postfix(
                ...
                Attack_Loss = '{:.3f}'.format(attack_cls_losses / (batch_idx + 1)),
...
def test(epoch):
    ...
    with tqdm(total=(len(testset) - len(testset) % args.bs), dynamic_ncols=True) as _tqdm:
        ...
        for batch_idx, (inputs, targets) in enumerate(testloader):
            ...
            ori_outputs, ori_attack_outputs, ori_r_outputs, ori_nr_outputs, ori_rec_outputs = net(inputs, is_attack = False, is_eval=True)
            adv_outputs, adv_attack_outputs, adv_r_outputs, adv_nr_outputs, adv_rec_outputs = net(adv_inputs, is_attack = True, is_eval=True)
