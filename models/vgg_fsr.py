...
class VGG(nn.Module):
    def __init__(self, features, num_classes=10, init_weights=True, tau=0.1, image_size=(32, 32)):
        ...
        self.attack_pool3 = features[7]
        self.attack_block4 = features[8]
        self.attack_pool4 = features[9]
        self.attack_avgpool = nn.AdaptiveAvgPool2d(1)
    ...
    def forward(self, x, is_attack, is_eval=False):
        r_outputs = []
        nr_outputs = []
        rec_outputs = []
        attack_output = []
        ...
        x = self.block2(x)
        x = self.pool2(x)
        x = self.block3(x) # shape: (b, 512, 4, 4)

        # attack 받은 input이면 FSR모듈
        if(is_attack):
            ...
            attack_output = self.attack_pool3(x)
            attack_output = self.attack_block4(attack_output)
            attack_output = self.attack_pool4(attack_output)
            attack_output = self.attack_avgpool(attack_output)
            attack_output = torch.flatten(attack_output, 1)
            attack_output = self.attack_classifier(attack_output)
        ...
        return x, attack_output, r_outputs, nr_outputs, rec_outputs
